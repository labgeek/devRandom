...
#
# SVN: $Id: FW.pm 1267 2006-07-26 18:14:32Z jdurick $
#
...
sub fwInit {
	...
	# get the vmid so that we can use it for PREROUTING logging
...
# Now lets set our default deny all policy, this drops all traffic for INPUT, FORWARD, and OUTPUT
		...
		#  Create a Drop_Log user-defined chain
		_createDropLog();
...
# we need to delete this rule too.  This loop applies the user-defined "in" chain.
	...
	for (my $i = 0 ; $i <= $#forwardrules ; $i++) {
			...
			&& ($forwardrules[$i]->{'jump'} eq $vin))
		{
			...
			$table->delete_entry("FORWARD", $forwardrules[$i]);
...
# we need to delete this rule too.  This loop applies the user-defined "out" chain.
	for (my $j = 0 ; $j <= $#forwardrules ; $j++) {
			...
			&& ($forwardrules[$j]->{'jump'} eq $vout))
		{
			...
			$table->delete_entry("FORWARD", $forwardrules[$j]);
		} else {
			print "No match with the rules, keep looking\n";
		}
	}
...
sub _flushChains {
	...
	my $table    = IPTables::IPv4::init('filter');
	my $natTable = IPTables::IPv4::init('nat');
	my ($natChain, $chain, $flushsuccess,) = q{};
...
# can never be deleted due to the fact that they are default chains.
		foreach $chain (@chains) {
			...
			$flushsuccess = $table->flush_entries($chain);
...
# Flushing the NAT table by looping through all the chains and flushing the rules
		foreach $natChain (@natChains) {
			$natTable->flush_entries($natChain);
			$natTable->delete_chain($natChain);
		}
...
# throughout the function.
	...
	if ((!defined($table)) || (!defined($natTable))) {
	   ...
	   # get list of chains, loop through that list and set the policy to ACCEPT
		...
		foreach $chain (@chains) {
			$log->info("$chain is set to ACCEPT policy now");

			# boolean value is returned from set_policy ($success)
			$success = $table->set_policy($chain, "ACCEPT");
		}
		foreach $natChain (@natChains) {
			$log->info("$natChain is set to ACCEPT policy now");
			$natTable->set_policy($natChain, "ACCEPT");
		}
		$table->commit();
		$natTable->commit();
	}
...
#pop(@chainlist);    # removes the FORWARD element in the list here
	foreach my $chainname (@chainlist) {
	 ...
	 # DROP all that does not match the rules within our INPUT and OUTPUT chains
		$table->append_entry("$chainname", { jump => "DROP" });
	}
...
# If the system is a linux box (we are using gentoo as our base operating system for
# all virtualization
	if ($linux) {
  ...
  # substitute nameserver with empty space so that we may isolate the IP address
		...
		$log->info("DNS failed because we are not on a linux box");
		return $linux;
	}
...
sub _createDropLog {
	...
	if ($table) {
		...
		$table->append_entry($chain, { jump => $drop });
	} else {
		$log->info("Table was not defined in _DROP_LOG user-defined chain");
		exit 0;
	}
	my $commit = $table->commit();
	return $commit;
}    # end of HoneyClient_log
...
sub _doFullBackup {
	my ($outputdir) = @_;
	my ($currenttime, $fh) = q{};
	my $outputfile = getVar(name => "backuplog");

	# logging objects
	my $log = get_logger("HoneyClient::Manager::FW");
	$log->info("Starting full backup to $outputfile");
...
# rename the outputfile by concatenating the current time on the end of the file
	rename($outputfile, '$outputfile$currenttime');
...
sub _sendMail {
	...
	my $subject = shift;
	my $body    = shift;
	open(SM, "|-", "/bin/mail", "-s", $subject, $to, "-f", $from);
	print SM $body, "\n";
	close(SM);
}
...
sub _getip {
	...
	if ($machine =~ m/Linux/i) {
		...
		print "You are not on a linux box, go away\n";
		exit(1);
	}
	if ($Linux) {
...
sub _getmac {
	...
	if ($machine =~ m/Linux/i) {
		...
		print "You are not on a linux box\n";
		exit(1);
	}
	if ($Linux) {
...
sub _resolveHost {
	...
	# Sanity check.
	if (!$query) {
		Carp::croak "Error: Unable to resolve host '" . $host . "'. "
...
<<<<<<< .mine
destroyTcpdump() - removes the corresponding  tcpdump .pcap files
=======
...
sub testConnect {
	...
	my $table = IPTables::IPv4::init('filter');
	my $dnsport = getVar(name => "dnsport");
	my ($success, $dnsname) = q{};

	# grabs the DNS setting from /etc/resolv.conf
	my @dnslist   = _getdns();
